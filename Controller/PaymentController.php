...
class PaymentController extends PaymentController_parent
{
    ...
    public function getPaymentErrorText()
    {
        ...
        $sMessage =  \OxidEsales\Eshop\Core\Registry::getLang()->translateString($sPaymentErrorText);
        return $sMessage;
    }
    ...
    protected function _triggerPaymentSpecificValidation()
    {
        ...
        if ( \OxidEsales\Eshop\Core\Registry::getConfig()->getRequestParameter( static::PARAM_PAYPAL_PLUS_PAYMENT_IS_ACTIVE ) ) {
            ...
            $oPayPalPaymentHandler = \OxidProfessionalServices\PayPalPlus\Core\Shop::getShop()->getFromRegistry( \OxidProfessionalServices\PayPalPlus\Core\PaymentHandler::class );
     ...
     *
     * @param \OxidEsales\Eshop\Core\Exception\StandardException $oException
     *
     * @return self
     */
    protected function _handlePaymentPaymentValidationErrors(\OxidEsales\Eshop\Core\Exception\StandardException $oException)
    {
        $oErrorHandler = $this->getShop()->getErrorHandler();
        $oErrorHandler->debug( $oException );
        $sMessage =  \OxidEsales\Eshop\Core\Registry::getLang()->translateString($oException->getMessage());
    ...
    protected function _setPayment($sPaymentId)
    {
        ...
        if (($oPaymentPrice instanceof \OxidEsales\Eshop\Core\Price) and
            ($oPaymentCost instanceof \OxidEsales\Eshop\Core\Price) and
         ...
         * \OxidEsales\Eshop\Application\Model\Basket::calculateBasket does not update the TS Cost without calling \OxidEsales\Eshop\Application\Model\Basket::setTsProductId prior to the update
         *
    ...
    {
        $this->_ajaxRespondWithMessage(\OxidProfessionalServices\PayPalPlus\Core\Shop::getShop()->getPayPalPlusConfig()->getInternalTransactionToken());
    }
