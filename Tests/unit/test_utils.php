...
class oxTestModules
{
    ...
    {
        return self::addFunction($class, preg_replace_callback('/^_/', 'p_', $fnc), "array(\$this, '$fnc')");
    }
...
if (!function_exists('getFileArr')) {
    function getFileArr()
    {
        ...
        if ($_ENV['PHP_FILE']) {
            ...
            $sTestOnlyFile = preg_replace_callback('/Test.php$/i', '', $sTestOnlyFile);
            $sTestOnlyFile = preg_replace_callback('/.php$/i', '', $sTestOnlyFile);
...
if (!function_exists('findphp')) {
    ...
    {
        $baseDir = preg_replace_callback('#/$#', '', $baseDir);
...
if (!function_exists('preg_stripper')) {
    ...
    {
        return preg_replace_callback('/.*/', '', $matches[0]);
    }
}
...
if (!function_exists('stripCodeLines')) {
    function stripCodeLines($sFile, $sCCarrayDir)
    {
        ...
        if (!$blFound) {
            ...
            $aFile = preg_replace_callback('#//.*#', '', $aFile);
            $aFile = preg_replace_callback('#/\*.*?\*/#sm', 'preg_stripper', $aFile);

            // for variables
            $aFile = preg_replace_callback('#(public|static|protected|private|var|\{|\}).*;#', '', $aFile);
            //for functions
            $aFile = preg_replace_callback('#(public|static|protected|private|var|\{|\})#', '', $aFile);

            $aFile = preg_replace_callback('#^class.*?$#m', '', $aFile);
            ...
            $aFile = preg_replace_callback('#^\$[a-zA-Z0-9_]+;$#m', '', $aFile);

            $aFile = preg_replace_callback('#^function[a-zA-Z0-9_]+\(.*?\)\{?$#m', '', $aFile);
            $aFile = preg_replace_callback('#.+#', '1', $aFile);

            $aFile = preg_replace_callback('#^$#m', '0', $aFile);
            $aFile = str_replace("\n", '', $aFile);
            $aCC = array();
...
class oxTestModuleLoader
{
    ...
    public static function loadModule()
    {
        ...
        if (count(self::$_aChain)) {
            ...
            \OxidEsales\Eshop\Core\Registry::get(\OxidEsales\Eshop\Core\UtilsObject::class)->setModuleVar("aDisabledModules", array());
        }
    }
    ...
    private static function _initMetadata($sPath)
    {
        ...
        if (isset($aModule["extend"]) && count($aModule["extend"])) {
            foreach ($aModule["extend"] as $sParent => $sPathToModule) {
                ...
                if (!class_exists($sFakeParent, false)) {
...
function importTestdataFile($sFileName)
{
    ...
    $sCall = "mysql -u" . $oConfig->getConfigParam("dbUser")
        ...
        ." 2>&1";  //MySQL 5.6 hack to hide awesome warning: Warning: Using a password on the command line interface can be insecure.
    exec($sCall);
}
