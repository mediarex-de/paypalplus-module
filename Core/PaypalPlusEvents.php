...
class PaypalPlusEvents extends \OxidProfessionalServices\PayPalPlus\Core\SuperCfg
{
    ...
    public function updateViews()
    {
        ...
        $oDbHandler = $this->getNew(\OxidEsales\Eshop\Core\DbMetaDataHandler::class);
    ...
    protected function _isInstalled()
    {
        ...
        $oPayment = $this->getNew(\OxidEsales\Eshop\Application\Model\Payment::class);
    ...
    protected function _createPaymentMethod()
    {
        ...
        if (!$oPayment->load($this->_getPayPalPlusMethodId())) {
            ...
            $oPayment->oxpayments__oxsort = new \OxidEsales\Eshop\Core\Field(-999); // Make the method topmost

            /** @var \OxidEsales\Eshop\Core\Language $oLanguage */
            $oLanguage = $this->getFromRegistry(\OxidEsales\Eshop\Core\Language::class);
            $aLanguages = $oLanguage->getLanguageIds();

            foreach ($aLanguages as $iLanguageId => $sAbbreviation) {
                $oPayment->setLanguage($iLanguageId);
                $oPayment->oxpayments__oxdesc = new \OxidEsales\Eshop\Core\Field('PayPal Plus');
                $oPayment->oxpayments__oxlongdesc = new \OxidEsales\Eshop\Core\Field('');
                $oPayment->save();
            }
        }
    }
    ...
    protected function _togglePaymentMethod($blIsActive)
    {
        ...
        if ($oPayment->load($this->_getPayPalPlusMethodId())) {
            $oPayment->oxpayments__oxactive = new \OxidEsales\Eshop\Core\Field((int) (bool) $blIsActive);
            $oPayment->save();
        }
    }
    ...
    protected function _assignAllCountries()
    {
        ...
        /** @var \OxidEsales\Eshop\Core\Model\ListModel $oCountries */
        $oCountries = $this->getNew(\OxidEsales\Eshop\Core\Model\ListModel);
        $oCountries->init(\OxidEsales\Eshop\Application\Model\Country::class);
    ...
    {
        /** @var \OxidEsales\Eshop\Core\Model\ListModel $oGroups */
    ...
    {
        /** @var \OxidEsales\Eshop\Application\Model\DeliverySetList $oShippingMethods */
        $oShippingMethods = $this->getNew(\OxidEsales\Eshop\Application\Model\DeliverySetList::class);
     ...
     *
     * @param \OxidEsales\Eshop\Core\Model\ListModel $oList
     ...
     */
    protected function _assignList(\OxidEsales\Eshop\Core\Model\ListModel $oList, $sRelationTable, array $aFieldsMap, $sRelationClass = \OxidEsales\Eshop\Core\Model\BaseModel::class)
    {
        if ($oList->count()) {
    ...
    {
        /** @var \OxidEsales\Eshop\Core\Model\BaseModel $oRelation */
        $oRelation = $this->getNew($sRelationClass);

        if ($sRelationClass === \OxidEsales\Eshop\Core\Model\BaseModel::class) {
            $oRelation->init($sRelationTable);
        }

        foreach ($aFieldsMap as $sField => $sValue) {
            $sFieldName = $sRelationTable . '__' . $sField;
            $oRelation->$sFieldName = new \OxidEsales\Eshop\Core\Field($sValue == '$' ? $oListItem->getId() : $sValue);
        }
    ...
    protected function _createPaypalWebhook()
    {
        ...
        try {
            ...
            \OxidEsales\Eshop\Core\Registry::get(\OxidEsales\EshopCommunity\Core\UtilsView::class)->addErrorToDisplay($oException);
        }
    }
