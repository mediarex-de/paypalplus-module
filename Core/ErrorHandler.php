...
class ErrorHandler extends \OxidProfessionalServices\PayPalPlus\Core\SuperCfg
{
    ...
    public function handelValidationMessage( $sMessageLangKey )
    {
        ...
        $oException = $oShop->getNew(\OxidEsales\Eshop\Core\Exception\ExceptionToDisplay::class);
        $oException->setMessage($sMessageLangKey);

        $oViewUtils = $oShop->getFromRegistry(\OxidEsales\Eshop\Core\UtilsView::class);
    ...
    public function parseError(Exception $oException, $blOnlyReturnOutput = false)
    {
        ...
        if ($oException instanceof \OxidEsales\Eshop\Core\Exception\StandardException) {
            $oException->debugOut();
        }
